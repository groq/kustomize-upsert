...
type ResourceSelector struct {
	...
	// Name of the target resource (supports regex patterns, e.g., "my-app.*")
	Name string `yaml:"name,omitempty"`
	
	// Namespace of the target resource (supports regex patterns)
...
func (t *UpsertTransformer) matchesTarget(item *yaml.RNode, target ResourceSelector) (bool, error) {
	...
	// Check name with regex support
	if target.Name != "" {
		matched, err := t.matchesPattern(item.GetName(), target.Name)
		if err != nil {
			return false, fmt.Errorf("invalid name pattern %q: %w", target.Name, err)
		}
		if !matched {
			return false, nil
		}
	...
	// Check namespace with regex support
	if target.Namespace != "" {
		matched, err := t.matchesPattern(item.GetNamespace(), target.Namespace)
		if err != nil {
			return false, fmt.Errorf("invalid namespace pattern %q: %w", target.Namespace, err)
		}
		if !matched {
			return false, nil
		}
...
// matchesPattern checks if a value matches a pattern (supports regex)
func (t *UpsertTransformer) matchesPattern(value, pattern string) (bool, error) {
	// Try regex first
	matched, err := regexp.MatchString(pattern, value)
	if err != nil {
		// If regex fails, fall back to exact string matching
