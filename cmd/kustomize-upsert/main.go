...
// matchesTarget checks if a resource matches the target selector using kustomize's selector logic
func (t *UpsertTransformer) matchesTarget(item *yaml.RNode, target ResourceSelector) (bool, error) {
	...
	// Create a selector ResId from the target's embedded Selector
	selectorId := resid.ResId{
		Gvk:       target.Gvk,
		Name:      target.Name,
		Namespace: target.Namespace,
	}
	
	// Use kustomize's built-in selection logic
	return resourceId.IsSelectedBy(selectorId), nil
}
